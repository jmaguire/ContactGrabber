...
class GoogleWebSearch(object):
    ...
    def get_result_urls(self):
        ...
        for g in glist:
            link = g.h3.a
            links = [url.a['href'] for url in g.findAll('h3')]
            index = 0
            link = links[index]
            while 'google' in link or 'wiki' in link:
                try:
                    link = links[index + 1]
                except:
                    break
            if 'google' in link or 'wiki' in link:
                continue
            url = self.getURL(link)
            if url == "":
                continue
            urls.append(url)
    ...
    def getURL(self,string):
        try:
            return re.match(r"\/url\?q=(.+)\&sa", string).groups()[0]
        except:
            return "";
if __name__ == "__main__":
    ...
    gws = GoogleWebSearch()
    gws.search('Lincoln Park  Michigan government', 1)
